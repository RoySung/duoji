...
When there exists at least one non-deleted settlement record that has one or more pending transfers, a distinct visual indicator/notification dot SHALL be displayed on the "已結算" (Settled) tab label.
...
- **WHEN** a settlement record has one or more pending transfers
- **THEN** the "已結算" (Settled) tab label SHALL display a visual notification dot
