...
When there exists at least one non-deleted settlement record that has one or more pending transfers, a distinct visual indicator/notification dot SHALL be displayed on the top-right corner of the "已結算" (Settled) tab label text.
...
- **WHEN** a settlement record has one or more pending transfers
- **THEN** the "已結算" (Settled) tab label SHALL display a visual notification dot at the top-right corner of the text
- **WHEN** all settlement records have all their transfers completed
- **THEN** the "已結算" (Settled) tab label SHALL NOT display a visual notification dot
